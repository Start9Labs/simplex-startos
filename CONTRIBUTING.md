...
This repo packages [SimpleX](https://github.com/simplex-chat/simplexmq) for StartOS.
...
SimpleX ships as two coordinated containers — `simplexchat/smp-server` and `simplexchat/xftp-server` — released together by the upstream project. They must be bumped together.

1. In `startos/manifest/index.ts`, update both `dockerTag` values to the new release:
...
3. Rebuild (`make`), sideload the `.s9pk`, and confirm both the SMP and XFTP servers come up healthy.
